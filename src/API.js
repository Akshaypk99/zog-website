...
const instance = axios.create({
  ...
  headers: {
    ...
    // 'Content-Security-Policy': "default-src 'self' https://zoggstudio.com; script-src 'self' 'unsafe-inline' https://zoggstudio.com;",
     //'X-Frame-Options':'DENY',  // Add X-Frame-Options header to prevent clickjacking
     //'X-Content-Type-Options': 'nosniff',                                                                                   //Permissions-Policy: <directive> <allowlist>  // no need for this. it is for camera mic etc permitions.
